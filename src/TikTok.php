...
class TikTok
{
    ...
    {
        $this->client_key = $params['client_key'] ?? null;
        $this->client_secret = $params['client_secret'] ?? null;
    }
    ...
    public function request($option = [])
    {
        ...
        if (stripos($option['url'], "https://") !== false) {
            curl_setopt($option['url'], CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($option['url'], CURLOPT_SSL_VERIFYHOST, false);
            curl_setopt($option['url'], CURLOPT_SSLVERSION, 1); //CURL_SSLVERSION_TLSv1
        }

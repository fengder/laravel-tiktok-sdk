...
class TikTokManager extends TikTok
{
    ...
    {
        return sprintf('%s?%s', self::API_OAUTH_URL. self::GET_WEB_OAUTH_CODE_URL, http_build_query(['client_key' => $client_key ?? $this->clientKey,'scope' => $scope, 'redirect_uri' => $redirect_uri, 'state' => $state, 'response_type' => 'code']));
    }
    ...
    {
        $data = $this->request(['url' => self::API_OAUTH_URL. self::GET_OAUTH_ACCESS_TOKEN_URL, 'method' => 'GET' , 'data' => ['client_key' => $client_key ?? $this->clientKey, 'client_secret' => $client_secret ?? $this->clientSecret, 'code' => $code, 'grant_type' => 'authorization_code']]);
